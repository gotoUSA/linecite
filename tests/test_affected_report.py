...
def test_markdown_lists_the_lines_to_re_read(sb, capsys):
    ...
    code, out = sb.run("affected", base, "docs/a.md", "--format", "markdown", capsys=capsys)
    assert code == 0, out
    assert out.startswith(f"**1 doc citation(s) point into code changed since `{base[:12]}`.**")
    assert "\n`docs/a.md`\n" in out
...
def test_markdown_links_doc_lines_under_the_link_base(sb, capsys):
    ...
    assert "- [line 3](https://github.com/o/r/blob/abc123/docs/a.md?plain=1#L3): " in out
...
def test_markdown_when_nothing_is_affected(sb, capsys):
    ...
    base = sb.commit()
    code, out = sb.run("affected", base, "docs/a.md", "--format", "markdown", capsys=capsys)
    assert code == 0 and out == f"No doc citation points into code changed since `{base[:12]}`.\n"
...
def test_unknown_rev_in_a_shallow_clone_says_how_to_fetch_it(
    ...
    assert code == 2
    assert "this is a shallow clone" in out and f"git fetch --depth=1 origin {base}" in out
...
def test_non_ascii_code_paths_are_diffed(sb, capsys):
    ...
    sb.write("docs/a.md", 'Lock: [café.py:10](../app/café.py#L10 "create_order: row.lock()").\n')
    base = sb.commit()
...
def test_shallow_hint_fetches_a_branch_by_its_name(sb, capsys, tmp_path_factory, monkeypatch):
    changed_since_base(sb)
    sb.commit()
    clone = tmp_path_factory.mktemp("shallow") / "c"
    sb.git("clone", "-q", "--depth", "1", sb.root.as_uri(), str(clone))
...
def test_sparse_changed_lines_are_listed_as_runs(sb, capsys):
    ...
    sb.write(
        "app/orders.py",
        ORDERS.replace("def create_order(self, items):", "def create_order(self, items, *, x=0):")
...
def test_cited_code_with_syntax_warnings_keeps_stderr_clean(sb, capsys):
    sb.write("app/rx.py", 'import re\n\n\ndef digits():\n    return re.compile("\d+")\n')
